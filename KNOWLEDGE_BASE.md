...
- **iOS:** first-class Tauri 2 target (`tauri ios init`, WKWebView shell), same codebase. Extra costs: macOS + Xcode required to build (CI macOS runner; Linux cannot produce an IPA), Apple Developer Program + signing + review, ATS exception for plain `ws://` to boat servers (`NSAllowsLocalNetworking`), iOS 14+ Local Network permission prompt (+ Bonjour declarations for mDNS discovery). For this app's Tauri config specifically, WKWebView has no Service Worker support in-app and no `navigator.wakeLock` — both need verifying against the actual `http://tauri.localhost` origin on a target iOS version before shipping, not assumed as a general WKWebView rule. Sequence iOS *last* — Android + Windows validate the shell first. To make iOS fall out for free: use official Tauri plugins (geolocation, keep-awake) rather than hand-rolled per-platform code.
