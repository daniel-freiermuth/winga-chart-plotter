...
- **iOS:** first-class Tauri 2 target (`tauri ios init`, WKWebView shell), same codebase. Extra costs: macOS + Xcode required to build (CI macOS runner; Linux cannot produce an IPA), Apple Developer Program + signing + review, ATS exception for plain `ws://` to boat servers (`NSAllowsLocalNetworking`), iOS 14+ Local Network permission prompt (+ Bonjour declarations for mDNS discovery). WKWebView is the strictest webview: no Service Workers in-app, no `navigator.wakeLock`. Sequence iOS *last* — Android + Windows validate the shell first. To make iOS fall out for free: use official Tauri plugins (geolocation, keep-awake) rather than hand-rolled per-platform code.
...
**Boundary rule:** cross the JS↔WASM boundary in **batches at event/data-change frequency, never per-item inside per-frame loops**. Perspective on the tax: a wasm-bindgen numeric call costs tens of ns — even 500 targets × 60 fps ≈ 1–2 ms/s aggregate. Per-item calls are pointless, not catastrophic; batched calls are perf-equivalent to TS. So "more Rust" is an economics question, not a perf rescue — the AIS hot store is already a flat `Float64Array`, the exact layout for a future zero-copy batch call if one is ever warranted.
...
- [ ] Signal K connection: direct WebSocket from browser, or proxied through Tauri Rust backend? *(2026-08 lean: direct WS — keeps browser and native code paths identical; Tauri stays packaging-only)*
