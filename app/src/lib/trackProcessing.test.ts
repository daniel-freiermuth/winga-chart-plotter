...
// Keep WASM readiness permanently pending so the `!ready` assertion is
...
// real-WASM code path and making the assertion untested depending on timing.
 ...
 *
 * These tests cover the pure-TS contract that runs before any WASM call: the
 * short-input early-return guards, and the fail-fast assertion that fires if
 * `main.ts`'s boot gate is ever bypassed and WASM isn't actually ready.
 */
describe('processTrack', () => {
  ...
  it('throws when called before WASM is ready (2-point input, distinct guard branch)', () => {
    // `wasmInit.ready` is mocked above to never resolve — this exercises the
    // fail-fast assertion deterministically, distinct from the `raw.length < 2`
    // guard above (2 points takes a different branch).
    const raw: [number, number][] = [[170, 10], [-170, 10]];
    expect(() => processTrack(raw)).toThrow(/WASM finished loading/);
  });
});
...
describe('processRouteCoords', () => {
  ...
  it('throws when called before WASM is ready (2-point input)', () => {
    const raw: [number, number][] = [[170, 10], [-170, 10]];
    expect(() => processRouteCoords(raw)).toThrow(/WASM finished loading/);
  });
});
