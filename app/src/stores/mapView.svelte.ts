...
export interface SavedMapView { center: [number, number]; zoom: number; bearing: number; pitch: number }

/** Oslo — used only on the very first run, before any view has ever been saved. */
const DEFAULT_VIEW: SavedMapView = { center: [10.75, 59.91], zoom: 10, bearing: 0, pitch: 0 };
...
/** Per-pane camera state: live center/zoom/bearing/pitch plus projection choice. */
export interface MapViewStore {
  ...
  syncView(c: [number, number], z: number, b: number, p: number): void;
...
export function createMapViewStore(lsSuffix = ''): MapViewStore {
  ...
  const init      = saved ?? { center: [...DEFAULT_VIEW.center] as [number, number], zoom: DEFAULT_VIEW.zoom, bearing: DEFAULT_VIEW.bearing, pitch: DEFAULT_VIEW.pitch };
  let _center     = $state<[number, number]>(init.center);
  let _zoom       = $state<number>(init.zoom);
  let _bearing    = $state<number>(init.bearing);
  let _pitch      = $state<number>(init.pitch);
  ...
  return {
    ...
    get pitch():   number           { return _pitch;   },
